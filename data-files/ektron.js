...
this.insertBefore(elem,this.firstChild);});},before:function(){return this.domManip(arguments,false,false,function(elem){this.parentNode.insertBefore(elem,this);});},after:function(){return this.domManip(arguments,false,true,function(elem){this.parentNode.insertBefore(elem,this.nextSibling);});},end:function(){return this.prevObject||jQuery([]);},find:function(selector){var elems=jQuery.map(this,function(elem){return jQuery.find(selector,elem);});return this.pushStack(/[^+>] [^+>]/.test(selector)||selector.indexOf("..")>-1?jQuery.unique(elems):elems);},clone:function(events){var ret=this.map(function(){if(jQuery.browser.msie&&!jQuery.isXMLDoc(this)){var clone=this.cloneNode(true),container=this.ownerDocument.createElement("div");container.appendChild(clone);return jQuery.clean([container.innerHTML],this.ownerDocument)[0];}else
...
obj=this.getElementsByTagName("tbody")[0]||this.appendChild(this.ownerDocument.createElement("tbody"));if(1==this.nodeType&&($ektron.isEditableElement(this)||"true"==this.getAttribute("contenteditable")))
...
return val;},handle:function(event){var val,ret,namespace,all,handlers;event=arguments[0]=jQuery.event.fix(event||window.event);namespace=event.type.split(".");event.type=namespace[0];namespace=namespace[1];all=!namespace&&!event.exclusive;handlers=(jQuery.data(this,"events")||{})[event.type];for(var j in handlers){var handler=handlers[j];if(all||handler.type==namespace){event.handler=handler;event.data=handler.data;try
...
{return(text+"").replace(Ektron.RegExp.ltrim,"").replace(Ektron.RegExp.rtrim,"");};if(typeof $ektron.fn.closest!="function")
{$ektron.fn.extend({closest:function(selector){return this.map(function(){var cur=this;while(cur&&cur.ownerDocument){if(jQuery(cur).is(selector))
return cur;cur=cur.parentNode;}});}});};$ektron.extend({ltrim:function(text){return(text+"").replace(Ektron.RegExp.ltrim,"");},rtrim:function(text){return(text+"").replace(Ektron.RegExp.rtrim,"");},addLoadEvent:function(fn)
{var oldOnLoad=window.onload;if(typeof window.onload!='function')
{window.onload=fn;}
else
{window.onload=function()
{oldOnLoad();fn();}}},ajaxCallback:function(uniqueId,data,callback,onexception)
{var options=("object"==typeof uniqueId?uniqueId:{uniqueId:uniqueId,data:data,success:callback,onexception:onexception});var ajaxSettings=$ektron.extend({uniqueId:"__Page",type:"POST",dataType:"html",data:"",success:function(){},onexception:null},options);var uniqueId=ajaxSettings.uniqueId;var data=ajaxSettings.data;if("object"==typeof data&&data!=null)
{data=$ektron.param(data);}
var successCallback=ajaxSettings.success;var onexception=ajaxSettings.onexception;var aryClientManager=[];$ektron(":hidden[id*='EktronClientManager']").each(function()
{if(this.value)
...
$ektron().trigger("EktronReady",["callback"]);}}));},isEditableElement:function(elem)
{if(!elem)return false;var bEditable=(true==elem.isContentEditable);if(elem.ownerDocument)
{if("on"==elem.ownerDocument.designMode)
{bEditable=true;var strContentEditable=elem.contentEditable;if("false"==strContentEditable)
{bEditable=false;}
else if(strContentEditable!="true")
{$ektron(elem).parents("[contenteditable]").each(function()
{var strContentEditable=this.contentEditable;if("true"==strContentEditable)return false;if("false"==strContentEditable)
{bEditable=false;return false;}});}}
...
{var eThis=$ektron(this);var content=eThis.contents();eThis.after(content).remove();ret=ret.concat(content.get());});return this.pushStack(ret);},ajaxCallback:function(uniqueId,data,callback,onexception)
{var me=this;var options=("object"==typeof uniqueId?uniqueId:{uniqueId:uniqueId,data:data,complete:callback,onexception:onexception});var ajaxSettings=$ektron.extend({success:function(){},complete:function(){},onexception:null},options);var successCallback=ajaxSettings.success;var completeCallback=ajaxSettings.complete;var onexception=ajaxSettings.onexception;$ektron.ajaxCallback($ektron.extend(ajaxSettings,{success:function(data)
{$ektron.each(me,function()
{$ektron(this).html(data);});try
{successCallback.apply(me,arguments);}
catch(ex)
{Ektron.OnException(me,onexception,ex,arguments,successCallback);}},complete:function()
{try
{completeCallback.apply(me,arguments);}
catch(ex)
{Ektron.OnException(me,onexception,ex,arguments,completeCallback);}},onexception:onexception}));return this;},makeIdentifiersUnique:function(makeUnique)
...
{Ektron.OnException.diagException(ex,arguments,$ektron.event.jquery_handle);return false;}};},$ektron.event);}
