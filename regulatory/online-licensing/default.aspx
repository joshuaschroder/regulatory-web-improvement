<?php
	$title= "Online Licensing Help Center - Regulatory Services" ;
	include '/home/codio/workspace/includes/header.html';
	include '/home/codio/workspace/includes/menus/no_menu.html'; 
?>

<hr/>
<p style="text-align: right;"><a title="Online Licensing" href="/vo/">Online Licensing System</a> | <a title="Contact Support" href="/regulatory/online-licensing/contact.aspx">Contact Us</a></p>
<hr/>
<table class="layout" width="100%">
	<tr>
		<td width="50%">
			<h2>Registration and Logging In</h2>
			<ul>
				<li><a title="Online Licensing Eligibility" href="/regulatory/online-licensing/registration-login.aspx#register">Online Licensing Eligibility</a></li>	
				<li><a title="Logging in" href="/regulatory/online-licensing/registration-login.aspx#login">Logging in</a></li>
				<li><a title="Lost Password" href="/regulatory/online-licensing/registration-login.aspx#password">Lost Password</a></li>
				<li><a title="Lost User ID" href="/regulatory/online-licensing/registration-login.aspx#user-id">Lost User ID</a></li>
				<li><a title="Can't Access Email on Record" href="/regulatory/online-licensing/registration-login.aspx#email">Can't Access Email on Record</a></li>
			</ul>
		</td>
		<td width="50%">
			<h2>Online Payment</h2>
			<ul>
				<li><a title="Pay for Online Applications" href="/regulatory/online-licensing/payment.aspx#pay-online">Pay for Online Applications</a></li>
				<li><a title="Third Party Payments" href="/regulatory/online-licensing/payment.aspx#third-party">Third Party Payments</a></li>
				<li><a title="Paying Later" href="/regulatory/online-licensing/payment.aspx#pay-later">Paying Later</a></li>
				<li><a title="Contacting Payment Support" href="/regulatory/online-licensing/payment.aspx#payment-support">Contacting Payment Support</a></li>
			</ul>
		</td>
	</tr>
	<tr>
		<td width="50%">
			<h2>Managing Licenses</h2>
			<ul>
				<li><a title="View Your Licenses" href="/regulatory/online-licensing/managing-licenses.aspx#view-licenses">View Your Licenses</a></li>
				<li><a title="Add a License to Your Account" href="/regulatory/online-licensing/managing-licenses.aspx#add-license">Add a License to Your Account</a></li>
			</ul>
		</td>
		<td width="50%">	
			<h2>Personal Information and Account Details</h2>
			<ul>
				<li><a title="Change of Address" href="/regulatory/online-licensing/account-details.aspx#change-address">Change of Address</a></li>
				<li><a title="Update Email Address" href="/regulatory/online-licensing/account-details.aspx#update-email">Update Email Address</a></li>
				<li><a title="Name Changes" href="/regulatory/online-licensing/account-details.aspx#name-change">Name Changes</a></li>
				<li><a title="DBA / Organization Name" href="/regulatory/online-licensing/account-details.aspx#dba">DBA / Organization Name</a></li>
			</ul>
		</td>
	</tr>
	<tr>
		<td width="50%">		
			<h2>License Applications and Renewals</h2>
			<ul>
				<li><a title="New License Applications" href="/regulatory/online-licensing/applications-renewals.aspx#apply-license">New License Applications</a></li>
				<li><a title="Renewal License Applications" href="/regulatory/online-licensing/applications-renewals.aspx#renew-license">Renewal License Applications</a></li>
				<li><a title="Check Status of Applications" href="/regulatory/online-licensing/applications-renewals.aspx#check-status">Check Status of Applications</a></li>
				<li><a title="Hard Copy Applications" href="/regulatory/online-licensing/forms.aspx">Hard Copy Applications</a></li>
			</ul>
		</td>
		<td width="50%">
			<h2>Technical Support</h2>
			<ul>
				<li><a title="Locked Out of Account" href="/regulatory/online-licensing/technical.aspx#locked-out">Locked Out of Account</a></li>
				<li><a title="Minimum Browser Requirements" href="/regulatory/online-licensing/technical.aspx#browser-requirements">Minimum Browser Requirements</a></li>
				<li><a title="Contact Support" href="/regulatory/online-licensing/contact.aspx">Contact Support</a></li>
			</ul>
		</td>
	</tr>
	
</table>
<hr/>

<?php include '/home/codio/workspace/includes/footer.html' ?>